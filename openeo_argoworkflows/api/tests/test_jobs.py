...
import pytest
from openeo_argoworkflows_api.jobs import ArgoJob, ArgoJobsRegister
...
def test_start_job(
    a_mock_user, a_mock_job, mock_links, mock_settings, mocked_validate_user
):
    create(a_mock_job)
...
def test_stop_queued_job_sets_canceled(
    mock_engine, a_mock_user, mock_links, mock_settings
):
...
def test_stop_running_job_calls_argo(
    mock_engine, a_mock_user, mock_links, mock_settings
):
...
def test_stop_finished_job_returns_400(
    mock_engine, a_mock_user, mock_links, mock_settings
):
