...
class ExtendedAuthenticator(Authenticator):
    ...
    def verify_signed_url(cls, url: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=401, detail="Signed URL could not be parsed."
            )
        except IndexError:
            raise HTTPException(
                status_code=401, detail="Signed URL could not be parsed."
...
# Make Depends(Authenticator.validate) in inherited upstream routes (e.g.
# JobsRegister.list_jobs) resolve to our argo-specific validation.
Authenticator.validate = ExtendedAuthenticator.validate
